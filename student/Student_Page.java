...
public void stu() {
	...
	final JFrame frame = new JFrame("Student");
	...
	//Label Student 
	JLabel la1 = new JLabel("Bus Pass Details ");
	...
	//Aadhar No
	JLabel la4 = new JLabel("Aadhar_no");
	la4.setBounds(30,300,120,24);
									...
									if(Id_No.length()>0) {
										if(Collage_Name.length()>0) {
											if(From.length()>0) {
												if(To.length()>0) {
													try {
														pst = con.prepareStatement("select * from stupass");
														rs = pst.executeQuery();
														while(rs.next()) {
														...
														if(Result=="Yes") {
															JOptionPane.showMessageDialog(frame, "Data is Already Recorded");
																
														}else {
															try {
																pst=con.prepareStatement("insert into stupass values(?,?,?,?,?,?,?,?,?,?,?,?,?)");
																...
																if(count>0) {
																	JOptionPane.showMessageDialog(frame,"Sucessfully Updated");
																	pay();
																}	
															}catch(Exception e1) {
																e1.printStackTrace();
...
public void pay() {
	...
	//head
	JLabel la1 = new JLabel("Student Pass Payment Page");
	...
	bu1.addActionListener(new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			//Exp
			JLabel la1 = new JLabel("Enter Exp Date");
			la1.setBounds(130,150,150,22);
			la1.setForeground(Color.white);
